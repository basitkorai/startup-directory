import { auth } from '@/auth'
import { client } from '@/sanity/lib/client'
import { AUTHOR_BY_ID_QUERY } from '@/sanity/lib/queries'
import { notFound } from 'next/navigation'
import React from 'react'
import Image from 'next/image'
import UserStartups from '@/components/UserStartups'

const User = async ({ params }: { params: Promise<{ id: string }> }) => {
  const id = (await params).id
  const session = await auth()

  const user = await client.fetch(AUTHOR_BY_ID_QUERY, { id })
  if (!user) return notFound()
  return (
    <>
      <section className="profile_container bg-[#FFFAFA]">
        <div className="profile_card !bg-pink-500">
          <div className="profile_title">
            <h3 className="text-24-black text-black uppercase text-center line-clamp-1">
              {user.name}
            </h3>
          </div>
          <Image
            src={user.image}
            alt={user.name}
            width={220}
            height={220}
            className="profie-image"
          />

          <p className="text-black text-30-bold mt-7 text-center">
            @{user?.username}
          </p>
          <p className="mt-1 text-center ">{user?.bio}</p>
        </div>

        <div className="flex-1 flex flex-col gap-5 lg:mt-5 ">
          <p className="text-30-bold">
            {session?.user?.id === id ? 'Your' : 'All'} Startups
          </p>
          <ul className="card_grid-sm">
            {/* User Startups */}
            <UserStartups id={id} />
          </ul>
        </div>
      </section>
    </>
  )
}

export default User
